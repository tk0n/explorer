    ...
    function Account () {
        this.validAddress = function (address) {
            return (
                ...
                // Todo: finish validation regex
                // typeof address === 'string' && address.match(/^[A-Za-z0-9]{50,52}$/g)
            );
        };
        ...
        this.getAccount = function (address, cb) {
            request.get({
                ...
                if (err || response.statusCode !== 200) {
                    ...
                    account.usd = exchange.ARKUSD(account.balance);
        ...
        request.get({
            ...
            if (err || response.statusCode !== 200) {
                ...
                async.forEach(body.accounts, function (a, cb) {
                    a.usd = exchange.ARKUSD(a.balance);

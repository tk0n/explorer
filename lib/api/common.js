...
module.exports = function (app, api) {
    ...
    this.getPriceTicker = function (error, success) {
        if (exchange.ARKBTC) {
            if (exchange.ARKBTC === '~') {
                return success({ success : true, btc_usd : (0).toFixed(8), ark_btc : (0).toFixed(8), ark_usd : (0).toFixed(2) });
            } else {
                return success({ success : true, btc_usd : exchange.BTCUSD, ark_btc : exchange.ARKBTC, ark_usd : exchange.ARKUSD(1) });
            }
        } else {
